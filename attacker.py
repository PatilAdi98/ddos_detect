...
def send_ping(source_ip: str,target_ip_address: str, number_of_packets_to_send: int = 4, size_of_packet: int = 65000):
    ip = IP(src=source_ip,dst=target_ip_address)
    icmp = ICMP()
    raw = Raw(b"X" * size_of_packet)
    p = ip / icmp / raw
    send(p, count=number_of_packets_to_send, verbose=0)
...
send_ping(sys.argv[1],destip, number_of_packets_to_send=100)
